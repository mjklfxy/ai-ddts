...
> **⚠️ 重要：本文档可能过时。** CLAUDE.md 是人工维护的参考，不是代码的 ground truth。
> 当文档描述与实际代码冲突时，**以代码为准**。遇到不确定的地方，直接读代码验证，不要盲信本文档。
...
临时推送子流程（`application/temp_push_runner.py`）：
```
配置正选 SKU 白名单 → 拉单 → SpecialSkuRule + GroupRule 过滤 → 按群拆分 → 生成 Excel → 上传 → 推送
```
临时推送使用独立的简化规则引擎（仅 2 条规则），产物存储在 `outputs/special_push/{TP-ID}/` 下。
...
| 目录 | 用途 |
|---|---|
| `application/` | 应用层：pipeline、scheduler、config_service、manual_runner、task_service、file_generator、order_splitter、temp_push_runner、special_push_order_store、各种 store |
| `config/` | 配置文件：`config.json`（默认）、`config.dev.json`、`config.prod.json`（不提交 git） |
...
| `infrastructure/` | 基础设施：jikeyun_client、qixin_client、message_adapter、cloud_warehouse_client、file_upload_client、kingdee_service、db_to_xlsx（RPA）、product_caller_sync_client、xlsx 解析器（region/sku/sku_group） |
| `input/` | RPA 导出的输入数据（`销售单查询.xlsx`） |
| `interfaces/` | FastAPI app + `static/` 前端（index.html、login.html、app.js、app.css） |
| `outputs/` | 运行产物：task_runs、exception_orders、pushed_orders、execution_logs、scheduler_state、supplier_mappings、order_files、special_push |
| `scripts/` | 工具脚本：csv_to_xlsx、manual_push_group、clear_task_data |
| `shared/` | 共享工具：`logging/logger.py`（JSON 日志 + 自动脱敏）、`env.py`（环境感知配置路径） |
| `tests/` | 测试（40 个文件，unittest 框架） |
...
| 数据类 | 关键字段 |
|---|---|
| `AppConfig` | 顶层配置，包含所有子配置；`schedules`（多定时任务 tuple）、`upload`、`product_caller_sync` |
| `TaskConfig` | `name`、`window_minutes` |
| `ScheduleConfig` | `enabled`、`run_at`（HH:MM）、`schedule_id`、`name`、`check_interval_seconds`、`stale_threshold_minutes` |
| `OrderSourceConfig` | `mode`：`mock` 或 `jikeyun` |
| `JikeyunConfig` | `api_url`、`app_key_env`、`app_secret_env`、`version`、`content_type`、`page_size`、`start_time_field`、`end_time_field`、`status_field`、`status_values`、`extra_params`、`page_index_base` |
| `RpaConfig` | `enabled`、`xlsx_path` |
| `KingdeeConfig` | `enabled`、`mode`（mock/http）、`api_url`、`token_env`、`timeout_seconds`、`tracking_id_fields`、`extra_headers` |
| `QixinConfig` | `mode`（mock/qixin）、`api_base_url`、`caller_id`、`secret_key_env`、`userid_api_url`、`timeout_seconds`、`push_mode`（file/link） |
| `RuleConfig` | 所有规则 `*_enabled` 开关 + 数据：`excluded_warehouses`、`excluded_skus`、`restricted_regions`、`sku_group_map`、`personal_order_suffix`、`allowed_order_prefixes`、`special_skus` |
...
| `ProductCallerSyncConfig` | `api_url`、`timeout_seconds` |
...
每次 git 提交必须在 commit message 中说明**本次做了什么**：
...
**禁止关键词搜索式调查。** 当用户要求检查某功能的逻辑时：

1. **找到代码入口**：从 API 路由（`interfaces/app.py`）或 CLI 入口（`main.py`）开始
2. **追踪调用链**：从入口函数 → application 层 → domain/infrastructure 层，逐层阅读
3. **读完整执行路径**：顺着函数调用读每一层的实际实现，不跳过
4. **理解后才回答**：只有在读完完整执行路径后才给出结论

❌ 错误：用户问"临时推送历史怎么工作的" → 直接搜"临时任务"/"历史"关键词
✅ 正确：找到 `GET /temp-push/orders` 路由 → 读 `list_temp_push_orders()` → 追到 `SpecialPushOrderStore.list_all()` → 读 store 实现 → 理解完整逻辑

**本质：从代码执行逻辑出发追代码，不从关键词出发猜代码。**

**工具：CodeGraph**（项目已初始化 `.codegraph/`），追踪调用链时优先使用：
- `codegraph_search` — 快速找符号位置
- `codegraph_trace` — 追踪调用链（跨层追踪，支持动态分派断点提示）
- `codegraph_context` — 获取某功能的完整上下文（入口 + 相关符号 + 关键代码）
...
| 决策 | 理由 |
|---|---|
| Pipeline 用 frozen dataclass 不用 dict | 不可变 + 类型安全 + 新增字段必须显式更新定义 |
| RuleEngine 独立于 Pipeline | 可复用（主流程和临时推送共用）、可独立测试、短路求值保证一致 |
| Store 用 JSON 不用 SQLite | 人类可读、无迁移负担、单写者简单性、`_optional_string()` 向后兼容 |
| 临时推送复制 Pipeline 逻辑 | 不同规则集（2 条 vs 6 条）、不同输出目录、需要 `file_prefix` 参数 |
| 依赖注入不用 import 直接调用 | 构造函数注入 `urlopen`/`sender`/`clock` → 测试时替换，生产时用真实现 |
...
推送阶段逐 batch 处理，一个失败不阻塞其他：
- `deliveries` 记录成功，`batch_failures` 记录失败
- 状态三态：`SUCCESS`（全成功）/ `PARTIAL`（部分成功）/ `FAILED`（全失败）
- 失败 batch 生成 `ExceptionOrder`，`rule_name="MessagePush"`

### 重试策略

| 组件 | 重试次数 | 退避策略 |
|---|---|---|
| 吉客云 API | 3 次 | 线性退避 `retry_delay_seconds * attempt` |
| 祺信消息 | `max_attempts`（默认 3） | `retry_interval_seconds`（默认 0，立即重试） |
| 重试每一步都记录日志（`message_send_failed` 含 attempt/max_attempts） |

### 错误记录

- `ExecutionLogRecord`：stage + result + summary + impact + suggestion + details
- `failure_reason` 格式：`"{ClassName}: {message}"`，截断 500 字符
- `failure_stage` 记录失败发生在哪个阶段（`message_push`、`kingdee_submit` 等）

### 优雅降级

- 金蝶禁用时跳过（`kingdee_enabled=False` → `KingdeeStatus.DISABLED`）
- RPA 导出失败不阻塞 API 拉单（异常被记录但重新抛出）
- user_id 解析失败回退到手机号（`pipeline.py` lazy resolution）

---

## 测试策略

### Mock 边界：在构造函数注入层，不 patch 内部实现

| 注入点 | Mock 方式 |
|---|---|
| `urlopen` | 注入假 HTTP 响应（`_block_network` 或 `fake_userid_urlopen`） |
| `sender` | `RecordingMessageSender` 记录调用 / `FailingMessageSender` 固定抛异常 |
| `clock` / `trace_id_generator` | 注入确定性 lambda |
| **永远不 mock** | domain 逻辑、规则评估、数据转换 |

### 手写 Recording Mock，不用 MagicMock
...
- 每个测试类用 `tmp/test_xxx/` 独立目录
- `setUp()` 删除旧文件，创建全新 store
- 确定性标识：固定 `TRACE-001`、固定时间

### 命名规范

`test_<行为>_<预期结果>`，例如：
- `test_no_passed_orders_skip_delivery_and_kingdee`
- `test_message_failure_returns_failed_push_status`
- `test_missing_supplier_rejects_purchase_request_before_transport`
...
| 陷阱 | 说明 |
|---|---|
| **SpecialSkuRule 忽略 enabled** | `special_skus` 非空时，`enabled` 标志无效，规则始终生效 |
| **异常订单连坐** | 一个 SKU 触发规则异常，同订单所有 SKU 都进异常列表（`"同订单其他SKU异常"`） |
| **`enabled_skus` → `excluded_skus` 语义反转** | 旧配置白名单被静默当作黑名单使用（`_string_list_with_legacy`） |
| **GroupRule 空 map + enabled=True** | 所有订单都会 ERROR（无 SKU 有配置群） |
| **XLSX 列布局硬编码** | `xlsx_reader.py` 列索引固定，RPA 工具改列顺序 → 地址数据静默丢失 |
| **吉客云签名必须 lowercase** | 整个签名串 `.lower()` 后再 MD5，否则"签名错误" |
| **JSON Store 无锁** | 单进程安全，多进程会损坏文件 |
| **分页启发式** | 总数恰好是 page_size 整数倍时多请求一次空页（正确但浪费） |
| **临时推送 userid URL 硬编码** | `temp_push_runner.py` 硬编码 URL，与 config 中的 `qixin.userid_api_url` 不同 |
...
4. 过期跳过：`stale_threshold_minutes`（默认 5 分钟），超过 `run_at` 该阈值则跳过本次触发
5. 后台循环：`application/scheduler_loop.py`，asyncio 驱动
6. 状态持久化：`outputs/scheduler_state.json`
...
| 祺信 | `infrastructure/qixin_client.py` | 企业微信群消息/文件推送（HMAC-SHA256 签名，支持 link/file 双模式） |
| 金蝶 | `infrastructure/kingdee_service.py` | 采购单提交 |
| 云仓 | `infrastructure/cloud_warehouse_client.py` | 供应商数据同步（CSV 解析 + 本地缓存） |
| 文件服务 | `infrastructure/file_upload_client.py` | Excel 文件上传托管 |
| 推送中心 | `infrastructure/product_caller_sync_client.py` | SKU 群推送配置同步 |
| RPA | `infrastructure/db_to_xlsx.py` | PyAutoGUI 桌面导出自动化（坐标硬编码，Flutter 应用） |
| XLSX 解析器 | `infrastructure/xlsx_region_parser.py` | 限发区域 Excel 导入（合并单元格 + 省级覆盖） |
| XLSX 解析器 | `infrastructure/xlsx_sku_parser.py` | 排除 SKU 列表 Excel 导入 |
| XLSX 解析器 | `infrastructure/xlsx_sku_group_parser.py` | SKU 群配置 Excel 导入（3 列 + 冲突检测） |
...
| GET | `/tasks/{trace_id}/payment` | 查询付款状态 |
| POST | `/tasks/{trace_id}/payment-receipt` | 上传付款回执 |
| GET | `/tasks/{trace_id}/pushed-orders/download` | 下载已推送订单 CSV |
...
| GET | `/temp-push/orders` | 列出临时推送批次 |
| GET | `/temp-push/{id}/orders/download` | 下载临时推送订单 CSV |
| GET | `/temp-push/{id}/exception-orders/download` | 下载临时推送异常订单 CSV |
| GET | `/temp-push/{id}/execution-logs/download` | 下载临时推送执行日志 CSV |
...
| POST | `/config/sku-groups/sync-caller-configs` | 同步 SKU 群推送配置 |
| POST | `/config/excluded-skus/upload-xlsx` | 导入排除 SKU |
...
| GET | `/execution-logs` | 执行日志列表（支持时间/阶段/结果筛选） |
| GET | `/execution-logs/download` | 下载执行日志 CSV |
| GET | `/supplier-mappings` | SKU-供应商映射 |
| PUT | `/supplier-mappings` | 更新 SKU-供应商映射 |
